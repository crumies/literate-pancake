...
final class DunenBLEManager: NSObject, ObservableObject {
    ...
    private func startPollTimer() {
        ...
        pollTimer = Timer.scheduledTimer(withTimeInterval: interval, repeats: true) { [weak self] _ in
            self?.developerStatus = "Polling every \(interval)s"
    ...
    private func updateDemo() {
        ...
        history.append(telemetry, currentValue: fakeCurrent)
    }
    ...
    private func addPacket(_ data: Data) {
        ...
        history.append(telemetry, currentValue: max(0, telemetry.torque * 3.0))
